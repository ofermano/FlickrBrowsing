...
+ (void)addPhotoDescription:(LTPhotoDescription *)description {
  ...
  NSUserDefaults *persistentMemory = [[NSUserDefaults standardUserDefaults] initWithSuiteName:kAppSuit];
...
+ (void)addItem:(NSString *)item forKey:(NSString *)key
  ...
  NSMutableArray *history = [LTPersistentPhotoHistory getArrayForKey:key forMemory:persistentMemory];
  if (!history)
    return;
  [history removeObject:item]; // Will work only if the item is in th array - in this case we will move it to the head of the array.
...
/// If it does - we need to convert it to mutable array as \c NSUserDefaults does not store mutable elements.
